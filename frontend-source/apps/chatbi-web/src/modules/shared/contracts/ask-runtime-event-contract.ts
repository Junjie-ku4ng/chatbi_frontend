import type { ChatProgressCategory, ChatStreamEvent } from '../../chat/runtime/chat-runtime-event-types'
...
export function formatAskRuntimeEventLabel(event: ChatStreamEvent) {
  ...
  if (event.event === 'clarification') {
    return event.data.message || '需要补充澄清信息'
  }
  if (event.event === 'done') {
    return '会话完成'
  }
  if (event.event === 'error') {
    return asString(event.data?.message) ?? '执行失败'
  }
  return '运行中'
}
