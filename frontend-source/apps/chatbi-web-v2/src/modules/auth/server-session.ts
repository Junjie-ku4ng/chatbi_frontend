...
export function resolveSessionTtlSeconds() {
  ...
  return Number.isFinite(raw) && raw > 0 ? Math.floor(raw) : DEFAULT_SESSION_TTL_SECONDS
}
...
export function resolveDefaultCredentials() {
  return {
    clientId: normalizeText(process.env.PA_WEB_AUTH_CLIENT_ID),
    clientSecret: normalizeText(process.env.PA_WEB_AUTH_CLIENT_SECRET),
    scope: normalizeText(process.env.PA_WEB_AUTH_SCOPE)
  }
}
...
export async function exchangeClientCredentials(input: { clientId: string; clientSecret: string; scope?: string }) {
  ...
  return {
    ...
    expiresIn: Number.isFinite(Number(result.expiresIn)) && Number(result.expiresIn) > 0 ? Math.floor(Number(result.expiresIn)) : resolveSessionTtlSeconds(),
