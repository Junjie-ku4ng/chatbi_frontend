...
export function buildAuthHeaders(): Record<string, string> {
  ...
  return {
    'x-user-id': resolveDevAuthHarnessUserId() ?? 'chatbi-web-v2',
    'x-roles': resolveDevAuthHarnessRoles()
  }
}
...
export function buildRequestContextHeaders(): Record<string, string> {
  ...
  const tenantId = normalizeTextHeader(resolveDevAuthHarnessTenantId())
  if (tenantId) {
    headers['Tenant-Id'] = tenantId
    headers['x-tenant-id'] = tenantId
  }

  const organizationId = normalizeTextHeader(resolveDevAuthHarnessOrganizationId())
  if (organizationId) {
...
export type SemanticModelDetail = Record<string, unknown> & {
  id: string
  name?: string
  type?: string
  catalog?: string
...
export type DataSourceDetail = Record<string, unknown> & {
  id: string
  name?: string
  type?: string
  typeCode?: string
...
export type RuntimeMetadataLevel = {
  name: string
  caption?: string
  semanticName?: string
  semanticRole?: string
...
export type RuntimeMetadataHierarchy = {
  name: string
  caption?: string
  semanticName?: string
  semanticRole?: string
  ...
  levels?: RuntimeMetadataLevel[]
}

export type RuntimeMetadataDimension = {
  name: string
  caption?: string
  semanticName?: string
  semanticRole?: string
  hierarchies?: RuntimeMetadataHierarchy[]
  levels?: string[]
}

export type RuntimeMetadataProjectionSummary = {
  modelId: string
  cube: string
  schemaVersion?: number
  measures?: string[]
  dimensions: RuntimeMetadataDimension[]
}

export type RuntimeMetadataProjectionResult = {
  projection: RuntimeMetadataProjectionSummary
  cache?: {
    version: number
    invalidationCursor?: string
  }
}

export type RuntimeMetadataMemberDescriptor = {
  dimension: string
  hierarchy?: string
  level?: string
  memberKey: string
...
export type RuntimeMetadataMemberPage = {
  items: RuntimeMetadataMemberDescriptor[]
  total: number
  limit?: number
  offset?: number
}

export type SemanticModelXmlaAxis = {
  name: string
  positions: string[][]
}

export type SemanticModelXmlaCell = {
  value?: unknown
  formatted?: string
}

export type SemanticModelXmlaResult = {
  axes: SemanticModelXmlaAxis[]
  cells: SemanticModelXmlaCell[]
}
...
export async function getSemanticModelById(modelId: string) {
  return apiRequest<SemanticModelDetail>(`/semantic-model/${encodeURIComponent(modelId)}`)
}

export async function getDataSourceById(dataSourceId: string) {
  return apiRequest<DataSourceDetail>(`/data-source/${encodeURIComponent(dataSourceId)}`)
}

export async function getSemanticModelRuntimeMetadata(modelId: string) {
  return apiRequest<RuntimeMetadataProjectionResult>(
    `/semantic-model/${encodeURIComponent(modelId)}/runtime-metadata`
  )
}

export async function listSemanticModelRuntimeMembers(
  modelId: string,
  query: {
    dimension?: string
    hierarchy?: string
  ...
  if (query.hierarchy) params.set('hierarchy', query.hierarchy)
  if (query.level) params.set('level', query.level)
  if (typeof query.limit === 'number') params.set('limit', String(query.limit))
  if (typeof query.offset === 'number') params.set('offset', String(query.offset))

  return apiRequest<RuntimeMetadataMemberPage>(
    `/semantic-model/${encodeURIComponent(modelId)}/runtime-metadata${
      params.toString() ? `/members?${params.toString()}` : '/members'
    }`
  )
}

export async function executeSemanticModelXmla(
  modelId: string,
  statement: string,
  options: {
    forceRefresh?: boolean
    operation?: 'query' | 'aggregate' | 'drill' | 'compare'
  } = {}
) {
  return apiRequest<SemanticModelXmlaResult>(`/semantic-model/${encodeURIComponent(modelId)}/xmla`, {
    method: 'POST',
    body: {
      statement: String(statement ?? '').trim().startsWith('<')
        ? statement
        : `<Envelope><Body><Execute><Command><Statement><![CDATA[${statement}]]></Statement></Command></Execute></Body></Envelope>`,
