...
export async function POST(request: NextRequest) {
  if (resolveAuthMode() !== 'bearer') {
    return NextResponse.json({ message: 'refresh is only available in bearer mode' }, { status: 400 })
  }
  ...
  if (fromCookie) {
    ...
    try {
      ...
      return NextResponse.json({ message: error instanceof Error ? error.message : 'missing refresh credentials' }, { status: 401 })
    }
  }
  ...
  try {
    ...
    return NextResponse.json({ message: error instanceof Error ? error.message : 'refresh failed' }, { status: 401 })
  }
}
